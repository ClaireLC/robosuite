...
from robosuite.environments.jr2_static_arm import JR2StaticArmEnv
...
class JR2StaticArmDoor(JR2StaticArmEnv):
    """
    ...
    def reward(self, action):
        ...
        print('hinge angle: {}'.format(door_hinge_angle))
        ...
        body_door_con = self.find_contacts(self.mujoco_robot.body_contact_geoms + self.mujoco_robot.gripper_contact_geoms + self.mujoco_robot.arm_contact_geoms, self.mujoco_objects["Door"].door_contact_geoms + self.mujoco_objects["Door"].handle_contact_geoms)
        body_door_con_num = len(list(body_door_con))
  

        # Arm links to door handle contacts 
        arm_handle_con = self.find_contacts(self.mujoco_robot.gripper_contact_geoms + self.mujoco_robot.arm_contact_geoms, self.mujoco_objects["Door"].handle_contact_geoms + self.mujoco_objects["Door"].door_contact_geoms)
        ...
        rew_door_angle = np.power(rew_door_angle,1)
        reward = rew_dist_to_handle + rew_door_angle + rew_handle_con + rew_body_door_con + rew_self_con + rew_eef_force

        #print("EEF force: {}".format(self._eef_force_measurement))
        #print("EEF torque: {}".format(self._eef_torque_measurement))

        # Target goal:
        robot_pos = np.array([self.robot_pose_in_world[0][3],self.robot_pose_in_world[1][3]])
        #print("robot pos: {}".format(robot_pos))
       
        dist_to_goal = np.linalg.norm(self.goal_pos - robot_pos)
        #print("dist to goal {}".format(dist_to_goal))
        #reward = 10 * (1 - np.tanh(dist_to_goal))
